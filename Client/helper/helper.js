...
export const projectDataFormatter = (data, contractAddrerss) => {
    const formattedData = {
...
export const groupContributors = (contributors) => {
    const contributorList = formatContribution(contributions);
    const contributorGroup = _.map(_.groupBy(contributorList, 'contributor'), (o, address) => { return { contributor: address, amount: _sumBy(o, 'amount') }})
...
export const connectWithWallet = async (onSuccess) => {
    if (window.ethereum) {
        ...
        window.alert(
            "Non-Ethereum browser detected. You should consider tryimg MetaMask"
        );
    }
};
